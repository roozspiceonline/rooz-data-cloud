...
def test_worker_queue_gate_defaults_off_and_requires_canary_run_lease() -> None:
    ...
    assert 'lease.work_kind != "RUN_START"' in service
    ...
    assert 'snapshot.get("request_queue_capability") != expected' in service
...
def test_worker_queue_is_lease_tenant_scoped_and_claim_token_bound() -> None:
    ...
    assert 'str(queue_id) != capability["queue_id"]' in service
