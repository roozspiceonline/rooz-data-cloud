...
def upgrade() -> None:
    ...
    op.execute('''CREATE OR REPLACE FUNCTION control.enforce_request_queue_tenancy() RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = control, identity, security, pg_temp AS $$ BEGIN IF NOT EXISTS (SELECT 1 FROM control.request_queues q WHERE q.id = NEW.queue_id AND q.organization_id = NEW.organization_id AND q.project_id = NEW.project_id) THEN RAISE EXCEPTION 'Request Queue tenancy mismatch' USING ERRCODE = '23514'; END IF; RETURN NEW; END; $$''')
...
def downgrade() -> None:
    ...
    op.execute('DROP FUNCTION IF EXISTS control.enforce_request_queue_request_immutable()')
