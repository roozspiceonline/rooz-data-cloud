...
def validate_kv_mutation(value: object) -> ValidatedKVMutation:
    ...
    if operation == "set":
        expected_fields = {
            "schema_version", "idempotency_key", "operation", "key",
            "expected_version", "content_type", "encoding", "value",
        }
    ...
    if operation == "set":
        content_type, encoding, normalized_value, value_bytes = _decoded_set_value(
    ...
    return ValidatedKVMutation(
        ...
        key=key,
        expected_version=expected_version,
        content_type=str(normalized["content_type"]) if operation == "set" else None,
        encoding=str(normalized["encoding"]) if operation == "set" else None,
